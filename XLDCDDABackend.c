...
void xld_cdda_print_disc_info(xld_cdread_t *disc)
{
	fprintf(stdout,"Drive: %s %s %s\n",disc->vendor,disc->product,disc->revision);
	fprintf(stdout,"Number of sessions: %d\n",disc->numSessions);
	fprintf(stdout,"Number of tracks: %d\n",disc->numTracks);
	int i;
	for(i=0;i<disc->numTracks;i++) {
		fprintf(stdout,"Track %02d (session %d)\n",i+1,disc->tracks[i].session);
		if(disc->tracks[i].type == kTrackTypeAudio) {
			if(disc->tracks[i].preEmphasis) fprintf(stdout,"  Audio with pre-emphasis\n");
			else fprintf(stdout,"  Audio\n");
		}
		else fprintf(stdout,"  Data\n");
		CDMSF msf = CDConvertLBAToMSF(disc->tracks[i].start/588);
		fprintf(stdout,"  start : %02d:%02d:%02d (%d)\n",msf.minute,msf.second,msf.frame,disc->tracks[i].start/588);
		msf = CDConvertLBAToMSF(disc->tracks[i].length/588-150);
		fprintf(stdout,"  length: %02d:%02d:%02d\n",msf.minute,msf.second,msf.frame);
	}
}

void xld_cdda_print_track_info(xld_cdread_t *disc, int track)
{
	int i = track - 1;
	fprintf(stdout,"Track %02d (session %d)\n",i+1,disc->tracks[i].session);
	if(disc->tracks[i].type == kTrackTypeAudio) {
